...
Color TextureProcessor::getTexel(float u, float v, float level) {
...
void TextureProcessor::generateMipmaps() {
...
Color TextureProcessor::bilinearFilter(float u, float v, int level) {
	...
	Color color1(m_Texture[level].get(x1, y1)),
...
Color TextureProcessor::trilinearFilter(float u, float v, float level) {

	int downLevel = level;
	Color color1 = bilinearFilter(u, v, downLevel), color2;
...
Color TextureProcessor::nearestFilter(float u, float v, int level) {
...
float TextureProcessor::Repeat(float u) {
...
float TextureProcessor::mirroredRepeat(float u) {
...
float TextureProcessor::clamp(float u) {

	return std::max(0.0f, std::min(1.0f, u));
}

int TextureProcessor::getTextureWidth(int level) {
	return m_Texture[level].width();
}

int TextureProcessor::getTextureHeight(int level) {
	return m_Texture[level].height();
}

void TextureProcessor::setTexture(TGAImage& texture) {

	m_Texture.resize(1);
	m_Texture[0] = texture;
}

void TextureProcessor::setWrapModeU(WrapMode Wrapmode) {
	m_WrapModeU = Wrapmode;
}

void TextureProcessor::setWrapModeV(WrapMode Wrapmode) {
	m_WrapModeV = Wrapmode;
}

void TextureProcessor::setFilterMode(FilterMode Fitlermode) {
