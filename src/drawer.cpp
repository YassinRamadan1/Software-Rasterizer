...
/*void Rasterizer::draw(const std::vector<Triangle>& triangles, const Texture& texture, const FrameBuffer& frameBuffer)
{
    switch (m_RenderMode)
    {
...
float Drawer::screenSpaceZ(float z) {

    return m_Framebuffer->near() + (z - m_Near) * (m_Framebuffer->far() - m_Framebuffer->near()) / (m_Far - m_Near);
}
...
float Rasterizer::calcMipmapLevel(fp46_16 du01, fp46_16 dv01, fp46_16 du10, fp46_16 dv10, fp46_16 width, fp46_16 height) {

    du01 *= width, dv01 *= height, du10 *= width, dv10 *= height;
    float l2 = double(std::max(du01 * du01 + dv01 * dv01, du10 * du10 + dv10 * dv10));

    return std::log2(l2) / 2.0f;
}

void Drawer::drawSolidOrthoTexture() {
...
void Drawer::drawSolid1(Triangle& triangle) {
    ...
    for (int y = ymin; y < ymax + 1; y++) {
        for (int x = xmin; x < xmax + 1; x++) {
            ...
            if (z_value < m_Framebuffer->getPixelDepth(x, y)) {
                ...
                // set the pixel color
                m_Framebuffer->setPixelColor(x, y, m_TextureProcessor->getTexel(double(u_interp), double(v_interp), mipmapLevel));

                // update the depth buffer
                m_Framebuffer->setPixelDepth(x, y, z_value);
            }
...
void Drawer::drawSolidPerspectiveTexture() {
    for (auto& triangle : (*m_Triangles)) {
...
void drawTex(const Triangle& triangle, const Texture& texture, FrameBuffer& frameBuffer)
{
    ...
    for (int i = xmin; i < xmax + 1; ++i)
    {
        for (int j = ymin; j < ymax + 1; ++j)
        {
            ...
            float mipmapLevel = texture.getMipmapLevel(textureCoords, textureCoords, textureCoords);
            frameBuffer.setPixelColor(i, j, texture.getTexel(textureCoords.x, textureCoords.y, mipmapLevel));
        }
    }
}
/*
void Drawer::drawSolid2(Triangle& triangle) {
...
*/
/*
void Drawer::drawSolidPerspectiveColor() {
    for (auto& triangle : (*m_Triangles)) {
...
void Drawer::drawSolid3(Triangle& triangle) {
    ...
    for (int y = ymin; y < ymax + 1; y++) {
        for (int x = xmin; x < xmax + 1; x++) {
            ...
            if (z_value < m_Framebuffer->getPixelDepth(x, y)) {
                ...
                // set the pixel color
                m_Framebuffer->setPixelColor(x, y, c1.toTGAColor());
...
void Drawer::drawSolidOrthoColor() {
    for (auto& triangle : (*m_Triangles)) {
...
void Drawer::drawSolid4(Triangle& triangle) {
    ...
    for (int y = ymin; y < ymax + 1; y++) {
        for (int x = xmin; x < xmax + 1; x++) {
            ...
            if (z_value < m_Framebuffer->getPixelDepth(x, y)) {
                ...
                // set the pixel color
                m_Framebuffer->setPixelColor(x, y, c1);
...
void Drawer::setFar(float far) {
    m_Far = far;
}

void Drawer::setNear(float near) {
    m_Near = near;
}
